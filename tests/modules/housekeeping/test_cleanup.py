...
def test_missing_feature_toggle_prevents_config_read(monkeypatch, caplog):
    monkeypatch.setattr(
        cleanup.feature_flags, "status", lambda key: _toggle_status(present=False)
    )
    monkeypatch.setattr(
        cleanup.recruitment,
        "get_config_value",
        lambda *_: (_ for _ in ()).throw(AssertionError("no config read")),
    )
...
def test_false_feature_toggle_prevents_scheduling(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        cleanup.feature_flags, "status", lambda key: _toggle_status(enabled=False)
    )
    assert cleanup.resolve_cleanup_config() is None
    assert (
        "cleanup disabled by Feature Toggle HOUSEKEEPING_CLEANUP_ENABLED=FALSE"
        in caplog.text
    )
...
def test_invalid_feature_toggle_prevents_scheduling(monkeypatch, caplog):
    monkeypatch.setattr(
        ...
        "status",
        lambda key: _toggle_status(invalid=True, invalid_value="maybe"),
    )
    assert cleanup.resolve_cleanup_config() is None
    assert (
        "required Feature Toggle HOUSEKEEPING_CLEANUP_ENABLED has invalid value"
        in caplog.text
    )
...
def test_enabled_feature_toggle_reads_config_not_toggle_from_config(monkeypatch):
    ...
    monkeypatch.setattr(
        cleanup.feature_flags,
        "status",
        lambda key: requested_toggles.append(key) or _toggle_status(enabled=True),
    )
    monkeypatch.setattr(
        cleanup.recruitment,
        "get_config_value",
        lambda key, default=None, **_kwargs: (
            config_keys.append(key) or values.get(key, default)
        ),
    )
    ...
    assert (
        cfg
        and cfg.tab_name == "CleanupRows"
        and cfg.run_every_hours == 6
...
class MsgWithReason(Msg):
    def __init__(self, content, *, author_id=10, pinned=False, hours_old=10):
        super().__init__(
            content, author_id=author_id, pinned=pinned, hours_old=hours_old
        )
        self.delete_reasons = []
...
def run_with_sheet(
    ...
    *,
    ...
    target = target or (
        Channel(messages) if target_type == "channel" else Thread(messages)
    )
...
def active_row(**overrides):
    data = {
        ...
        "target_name": "",
        "parent_name": "",
        "cleanup_mode": "bot_messages_only",
        "min_age_hours": "1",
        "last_checked_at_utc": "",
        "last_deleted_count": "",
...
def test_run_cleanup_uses_async_safe_sheets_helper_for_read_and_write(monkeypatch):
    ...
    monkeypatch.setattr(cleanup.async_core, "aget_worksheet", fake_sheet)
    monkeypatch.setattr(
        cleanup.async_core, "a_to_thread_with_backoff", fake_to_thread_with_backoff
    )
    monkeypatch.setattr(
        cleanup.async_core, "acall_with_backoff", fail_acall_with_backoff
    )
    monkeypatch.setattr(cleanup.runtime_helpers, "send_log_message", fake_log)
    monkeypatch.setattr(cleanup, "_resolve_any", fake_resolve)
...
def test_blank_target_type_resolved_and_dry_run_deletes_nothing(monkeypatch):
    ws, msgs, _target = run_with_sheet(
        monkeypatch, [active_row()], [Msg("bot", author_id=99)]
    )
...
def test_invalid_rows_write_statuses(monkeypatch):
    rows = [
        active_row(target_id="abc"),
        active_row(cleanup_mode="bad"),
        active_row(min_age_hours="bad"),
    ]
...
def test_cleanup_modes_and_min_age_respected(monkeypatch):
    msgs = [
        Msg("human", author_id=1),
        Msg("!cmd", author_id=1),
        Msg("bot", author_id=99),
        Msg("pinned", author_id=99, pinned=True),
        Msg("young", author_id=99, hours_old=0),
    ]
    ws, msgs, _target = run_with_sheet(
        monkeypatch,
        [active_row(cleanup_mode="bot_messages_and_commands", min_age_hours="1")],
        msgs,
        dry_run=False,
    )
...
def test_all_non_pinned_skips_pinned(monkeypatch):
    ...
    run_with_sheet(
        monkeypatch,
        [active_row(cleanup_mode="all_non_pinned", min_age_hours="0")],
        msgs,
        dry_run=False,
    )
...
def test_bot_messages_only_matches_own_bot_messages(monkeypatch):
    ...
    assert (
        cleanup._matches_mode(Msg("bot", author_id=99), "bot_messages_only", Bot())
        is True
    )
...
def test_bot_messages_only_matches_other_discord_bot_accounts(monkeypatch):
    ...
    assert (
        cleanup._matches_mode(
            Msg("bot", author_id=42, author_bot=True), "bot_messages_only", Bot()
        )
        is True
    )
...
def test_bot_messages_only_matches_configured_common_bot_role(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "get_config_value",
        lambda key, default=None, **_kwargs: (
            "12345" if key == cleanup.CONFIG_BOT_ROLE_IDS else default
        ),
    )
    assert (
        cleanup._matches_mode(
            Msg("role bot", author_id=42, roles=[role]), "bot_messages_only", Bot()
        )
        is True
    )
...
def test_bot_messages_only_rejects_non_bot_user_without_role(monkeypatch):
    ...
    assert (
        cleanup._matches_mode(
            Msg("human", author_id=1, author_bot=False, roles=[]),
            "bot_messages_only",
            Bot(),
        )
        is False
...
def test_bot_messages_only_does_not_match_webhook_messages(monkeypatch):
    ...
    assert (
        cleanup._matches_mode(
            Msg("", author_id=1, webhook_id=555), "bot_messages_only", Bot()
        )
        is False
    )
...
def test_webhook_inclusive_modes_match_webhook_messages(monkeypatch):
    ...
    assert (
        cleanup._matches_mode(
            Msg("", author_id=1, webhook_id=555), "bot_and_webhook_messages_only", Bot()
        )
        is True
    )
    assert (
        cleanup._matches_mode(
            Msg("", author_id=1, webhook_id=555),
    ...
    assert (
        cleanup._matches_mode(
            Msg("!cmd", author_id=1), "bot_webhook_messages_and_commands", Bot()
        )
        is True
    )


def test_automod_system_messages_only_matches_conservative_automod_system(monkeypatch):
    monkeypatch.setattr(
        cleanup.recruitment,
        "get_config_value",
        lambda key, default=None, **_kwargs: default,
    ...
    assert (
        cleanup._matches_mode(
            Msg("", author_id=1, message_type=automod_type),
...
def test_automod_system_fallback_uses_indicators_or_known_phrase(monkeypatch):
    monkeypatch.setattr(
        cleanup.recruitment,
        "get_config_value",
        lambda key, default=None, **_kwargs: default,
    ...
    assert (
        cleanup._matches_mode(
            Msg("", author_id=1, message_type=generic_system_type, automod_rule_id=123),
            "automod_system_messages_only",
            Bot(),
        )
        is True
    ...
    assert (
        cleanup._matches_mode(
            Msg(
                ...
                author_id=1,
                message_type=generic_system_type,
            ),
            "automod_system_messages_only",
            Bot(),
        )
...
def test_automod_system_messages_only_rejects_generic_user_bot_webhook_and_pinned(
    monkeypatch,
):
    monkeypatch.setattr(
        cleanup.recruitment,
    ...
    assert (
        cleanup._matches_mode(
            Msg("human", author_id=1), "automod_system_messages_only", Bot()
        )
        is False
    )
    assert (
        cleanup._matches_mode(
            Msg("bot", author_id=42, author_bot=True),
            "automod_system_messages_only",
            Bot(),
        )
        is False
    ...
    assert (
        cleanup._matches_mode(
            Msg("", author_id=1, webhook_id=555), "automod_system_messages_only", Bot()
        )
        is False
    )
    assert (
        cleanup._matches_mode(
            Msg("", author_id=1, message_type=generic_system_type),
            "automod_system_messages_only",
            Bot(),
        )
        is False
    ...
    assert (
        cleanup._matches_mode(
            Msg("", author_id=1, pinned=True, message_type=automod_type),
            "automod_system_messages_only",
            Bot(),
        )
        is False
...
def test_commands_only_uses_project_prefix_fallback_not_hardcoded(monkeypatch):
    ...
    assert (
        cleanup._matches_mode(Msg("?cleanup run", author_id=1), "commands_only", bot)
        is True
    )
    assert (
        cleanup._matches_mode(Msg("!cleanup run", author_id=1), "commands_only", bot)
...
def test_combined_mode_uses_bot_or_command_helpers(monkeypatch):
    ...
    assert (
        cleanup._matches_mode(
            Msg("bot", author_id=99), "bot_messages_and_commands", Bot()
        )
        is True
    )
    assert (
        cleanup._matches_mode(
            Msg("!cmd", author_id=1), "bot_messages_and_commands", Bot()
        )
        is True
    )
    assert (
        cleanup._matches_mode(
            Msg("normal", author_id=1), "bot_messages_and_commands", Bot()
        )
        is False
    )
...
def test_pinned_bot_and_command_messages_do_not_match_for_deletion(monkeypatch):
    ...
    assert (
        cleanup._matches_mode(
            Msg("bot", author_id=99, pinned=True), "bot_messages_only", Bot()
        )
        is False
    )
    assert (
        cleanup._matches_mode(
            Msg("!cmd", author_id=1, pinned=True), "commands_only", Bot()
        )
        is False
    )
    assert (
        cleanup._matches_mode(
            Msg("", author_id=1, pinned=True, webhook_id=555),
...
def test_zero_match_diagnostics_counts_webhooks_empty_content_and_bounded_authors(
    monkeypatch, caplog
):
    monkeypatch.setattr(
        cleanup.recruitment,
...
def test_startup_validation_writeback_does_not_delete_when_dry_run_false(monkeypatch):
    ...
    ws, _, _target = run_with_sheet(
        monkeypatch, [active_row()], msgs, dry_run=False, startup_validation=True
    )
    updates = update_map(ws)
    assert updates["C2"] == "thread"
    assert updates["H2"]
...
def test_blank_target_type_resolving_to_channel_writes_channel(monkeypatch):
    ws, _msgs, _target = run_with_sheet(
        monkeypatch,
        [active_row(target_type="")],
        [Msg("bot", author_id=99)],
        target_type="channel",
    )
    updates = update_map(ws)
...
def test_channel_dry_run_counts_candidates_and_deletes_nothing(monkeypatch):
    msgs = [Msg("bot", author_id=99), Msg("human", author_id=1)]
    ws, msgs, _target = run_with_sheet(
        monkeypatch, [active_row(target_type="channel")], msgs, target_type="channel"
    )
    updates = update_map(ws)
    assert [m.deleted for m in msgs] == [False, False]
    assert updates["I2"] == "0"
...
def test_channel_cleanup_modes_match_thread_modes(monkeypatch):
    cases = [
        (
            "all_non_pinned",
            [Msg("human", author_id=1), Msg("pinned", author_id=99, pinned=True)],
            [True, False],
        ),
        (
            "bot_messages_only",
            [Msg("human", author_id=1), Msg("bot", author_id=99)],
            [False, True],
        ),
        (
            "commands_only",
            [Msg("normal", author_id=1), Msg("!cmd", author_id=1)],
            [False, True],
        ),
        (
            "bot_messages_and_commands",
            [
                Msg("normal", author_id=1),
                Msg("!cmd", author_id=1),
                Msg("bot", author_id=99),
            ],
            [False, True, True],
    ...
    for mode, msgs, expected in cases:
        run_with_sheet(
            monkeypatch,
            [active_row(target_type="channel", cleanup_mode=mode, min_age_hours="0")],
            msgs,
            dry_run=False,
            target_type="channel",
        )
...
def test_channel_like_history_target_resolves_as_channel_without_child_traversal(
    monkeypatch,
):
...
def test_unsupported_target_type_still_applies_to_unsupported_resolved_objects(
    monkeypatch,
):
    ws, _msgs, _target = run_with_sheet(
        monkeypatch,
...
def test_cleanup_headers_and_config_keys_unchanged():
    ...
    assert cleanup.REQUIRED_CONFIG_KEYS == (
        cleanup.CONFIG_TAB,
...
def test_cleanup_manual_command_is_admin_only():
    admin_checks = [
        check
        for check in cleanup.CleanupCog.cleanup_run.checks
...
def test_cleanup_manual_command_handles_failure(monkeypatch, caplog):
    ...
    assert replies == [
        ("Cleanup run started.", False),
        ("Cleanup run failed: RuntimeError. See logs.", False),
    ]
    assert sent_logs == [
        "🧹 cleanup manual run requested: actor=1234 channel=5678",
    ...
    assert (
        "cleanup manual run failed: error_type=RuntimeError error=boom" in caplog.text
    )
    failure_record = next(
        record
        for record in caplog.records
        if record.message.startswith("cleanup manual run failed")
    )
...
def test_run_cleanup_read_values_api_error_sets_actionable_first_error(monkeypatch):
    ...
    assert (
        cleanup._format_summary_error(summary)
        == "APIError: quota exceeded at read_values"
    )
...
def test_run_cleanup_unexpected_failure_logs_stage_and_context(monkeypatch, caplog):
    ...
    record = next(
        record
        for record in caplog.records
        if record.message.startswith("cleanup run unexpected failure")
    )
