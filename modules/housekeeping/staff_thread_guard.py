...
def _validate_rule(row: Mapping[str, Any], *, row_number: int) -> GuardRule | None:
    ...
    if not guard_id or not thread_id:
        log.warning(
            "staff thread guard row skipped: row=%s missing guard_id/thread_id",
            row_number,
        )
        return None
    if action not in {"delete", "redirect"}:
        log.warning(
            "staff thread guard row skipped: row=%s invalid action=%r",
            row_number,
            action,
        )
        return None
    if action == "redirect" and not redirect_target_id:
        log.warning(
            "staff thread guard row skipped: row=%s redirect_target_id missing",
            row_number,
        )
        return None
    ...
    redirect_body_text = _row_value(row, "redirect_body_text")
    failure_text = _row_value(row, "failure_text")

    if not warning_text or not failure_text:
        log.warning(
            "staff thread guard row skipped: row=%s warning/failure text missing",
            row_number,
        )
        return None
    if action == "redirect" and (
        not redirect_notice_text or not redirect_header_text or not redirect_body_text
    ):
        log.warning(
            "staff thread guard row skipped: row=%s redirect text missing",
            row_number,
        )
        return None
    ...
    if timeout_after_offenses > 0 and (
        offense_window_minutes <= 0 or timeout_seconds <= 0 or not timeout_text
    ):
        log.warning(
    ...
    return GuardRule(
        ...
        warning_delete_after_seconds=_parse_int(
            _row_value(row, "warning_delete_after_seconds")
        ),
        ...
        redirect_body_text=redirect_body_text,
...
async def load_rules(*, force: bool = False) -> dict[int, GuardRule]:
    ...
    if (
        not force
        and _RULE_CACHE_LOADED_AT
        and now - _RULE_CACHE_LOADED_AT < RULE_CACHE_TTL_SECONDS
    ):
    ...
    async with _RULE_CACHE_LOCK:
        ...
        rules_tab, _ = tabs
        try:
            records = await async_core.afetch_records(
                recruitment.get_recruitment_sheet_id(), rules_tab
            )
        ...
        for row_number, row in enumerate(records, start=2):
            ...
            if rule.guard_id in guard_ids:
                log.warning(
                    "staff thread guard duplicate guard_id skipped: %s", rule.guard_id
                )
                continue
            if rule.thread_id in rules:
                log.warning(
                    "staff thread guard duplicate thread_id skipped: %s", rule.thread_id
                )
                continue
...
def _offense_lock(guard_id: str, user_id: int) -> asyncio.Lock:
    ...
    if key not in _OFFENSE_LOCKS:
        _OFFENSE_LOCKS[key] = asyncio.Lock()
    return _OFFENSE_LOCKS[key]
...
def _state_from_values(
    ...
    missing = [name for name in OFFENSE_HEADERS if name not in headers]
    if missing:
        raise ValueError(
            "staff thread guard offenses tab missing headers: " + ", ".join(missing)
        )
...
def _advance_offense(
    state: OffenseState | None,
    rule: GuardRule,
    *,
    user_id: int,
    now: datetime,
    ...
    window_started = _parse_utc(state.window_started_at_utc)
    within_window = bool(
        rule.offense_window_minutes > 0
        and window_started
        and now - window_started < timedelta(minutes=rule.offense_window_minutes)
    )
    if within_window:
...
async def _load_and_advance_state(
    rule: GuardRule, message: discord.Message
) -> OffenseState:
    tabs = await _tab_names()
    if tabs is None:
        return _advance_offense(
            None, rule, user_id=message.author.id, now=_utc_now()
        )
    _, offenses_tab = tabs
    values = await async_core.afetch_values(
        recruitment.get_recruitment_sheet_id(), offenses_tab
    )
    state = _state_from_values(
        values, guard_id=rule.guard_id, user_id=message.author.id
    )
    return _advance_offense(
        state, rule, user_id=message.author.id, now=_utc_now()
    )
...
def _message_parts(message: discord.Message) -> tuple[str, str]:
    content = str(message.content or "").strip()
    if len(content) > 2800:
        content = content[:2799] + "…"
    if not content:
        content = "*No text content.*"

    links: list[str] = []
    for attachment in list(getattr(message, "attachments", []) or [])[:8]:
...
def _format_values(
    ...
    *,
    ...
    message_content, attachment_links = _message_parts(message)
    redirect_label = getattr(redirect_target, "mention", None)
    if not redirect_label:
        redirect_label = (
            f"#{getattr(redirect_target, 'name', 'configured destination')}"
            if redirect_target is not None
            else "the proper deck"
        )
    source_label = getattr(source, "mention", None) or f"#{getattr(source, 'name', 'this thread')}"
    return {
        "user": getattr(author, "mention", None)
        or getattr(author, "display_name", "matey"),
        "user_name": getattr(author, "display_name", None)
        or getattr(author, "name", "matey"),
...
async def _resolve_redirect_target(
    bot: commands.Bot, rule: GuardRule
...
async def _redirect_message(
    ...
    *,
    ...
    values = _format_values(
        rule, message, offense_count=offense_count, redirect_target=target
    )
    embed = discord.Embed(
        title=_safe_format(rule.redirect_header_text, values)[:256],
        description=_safe_format(rule.redirect_body_text, values)[:4096],
    )
    try:
        await target.send(
        ...
        log.warning(
            "staff thread guard redirect send failed: guard_id=%s",
            rule.guard_id,
            exc_info=True,
        )
        return False, target
...
async def _apply_timeout(
    rule: GuardRule, message: discord.Message, *, offense_count: int
) -> bool:
    if (
        rule.timeout_after_offenses <= 0
        or offense_count < rule.timeout_after_offenses
    ):
        return False
...
async def _send_notice(
    ...
    *,
    ...
    embed = discord.Embed(description=_safe_format(text, values))
    try:
        ...
        log.warning(
            "staff thread guard warning send failed: guard_id=%s",
            rule.guard_id,
            exc_info=True,
        )
...
async def handle_message(bot: commands.Bot, message: discord.Message) -> bool:
    ...
    if not _feature_enabled() or getattr(message, "guild", None) is None:
        return False
    ...
    async with _offense_lock(rule.guard_id, message.author.id):
        try:
            ...
            log.exception(
                "staff thread guard offense state load failed: guard_id=%s",
                rule.guard_id,
            )
            state = _advance_offense(
                None, rule, user_id=message.author.id, now=_utc_now()
            )
        ...
        if rule.action == "redirect":
            redirected, redirect_target = await _redirect_message(
                bot, rule, message, offense_count=offense_count
            )
            action_ok = (
                await _delete_original(rule, message) if redirected else False
            )
        else:
            action_ok = await _delete_original(rule, message)

        timed_out = await _apply_timeout(
            rule, message, offense_count=offense_count
        )
        if timed_out:
            ...
            state.window_started_at_utc = ""
            state.last_action = (
                "timeout" if action_ok else "timeout_action_failed"
            )
        else:
            state.last_action = (
                rule.action if action_ok else f"{rule.action}_failed"
            )
        ...
        try:
            ...
            log.exception(
                "staff thread guard offense state save failed: guard_id=%s",
                rule.guard_id,
            )
...
def install(bot: commands.Bot) -> None:
    ...
    if not callable(original):
        raise RuntimeError(
            "bot.on_message unavailable for staff thread guard installation"
        )
