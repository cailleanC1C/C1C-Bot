...
class KnockoutService:
    def __init__(self, sheet_id: str, registration_repository=None, qualification_repository=None, clock=None):
    ...
    async def generate_quarterfinal_preview(self, actor_id: str) -> QualificationSnapshot:
        ...
        pairs = [(seeds[0], seeds[7]), (seeds[3], seeds[4]), (seeds[1], seeds[6]), (seeds[2], seeds[5])]
        return await self._create_preview(actor_id, "quarterfinal", pairs)

    async def generate_next_preview(self, actor_id: str, stage: str) -> QualificationSnapshot:
        ...
        previous_matches = sorted(
            [row for row in matches if _text(row.get("tournament_id")) == tid and _text(row.get("round_id")) == f"{tid}-{previous['suffix']}"],
            key=lambda row: int(_text(row.get("match_number")) or 0),
        )
        expected = 4 if stage == "semifinal" else 2
        if len(previous_matches) != expected or any(_text(row.get("status")) not in MATCH_TERMINAL_STATUSES for row in previous_matches):
        ...
        if any(not uid for uid, _ in winners):
            raise RegistrationError("A double-forfeit knockout result requires organizer resolution before advancement")
        if stage == "semifinal":
            pairs = [(_player(winners[0]), _player(winners[1])), (_player(winners[2]), _player(winners[3]))]
        else:
            pairs = [(_player(winners[0]), _player(winners[1]))]
        return await self._create_preview(actor_id, stage, pairs)
    ...
    async def approve_and_open(self, actor_id: str, stage: str) -> QualificationSnapshot:
        ...
        async with _locks[(self.sheet_id, tid)]:
            ...
            round_matches = [row for row in old_matches if _text(row.get("tournament_id")) == tid and _text(row.get("round_id")) == round_id]
            ...
            for row in matches:
                if _text(row.get("tournament_id")) == tid and _text(row.get("round_id")) == round_id:
                    row.update(status="published", published_at_utc=now, deadline_at_utc=deadline)
            await self.repository.persist_state(rounds, matches, previous_rounds=old_rounds, previous_matches=old_matches)
            await self._audit(tid, actor_id, "knockout_round_opened", {"stage": stage, "round_id": round_id}, now)
            return QualificationSnapshot(opened, tuple(row for row in matches if _text(row.get("round_id")) == round_id and _text(row.get("tournament_id")) == tid))
    ...
    async def complete_tournament(self, actor_id: str) -> dict[str, object]:
        ...
        async with _locks[(self.sheet_id, tid)]:
            ...
            final_matches = [row for row in matches if _text(row.get("tournament_id")) == tid and _text(row.get("round_id")) == f"{tid}-F"]
            ...
            now = utc_iso(self.clock().astimezone(UTC))
            # Tournament-row persistence is deliberately delegated to the existing lifecycle service;
            # this return payload is the canonical competitive completion summary.
            summary = {
                "tournament_id": tid,
            ...
            await self._audit(tid, actor_id, "knockout_competition_completed", summary, now)
            return summary

    async def _create_preview(self, actor_id: str, stage: str, pairs) -> QualificationSnapshot:
        meta = _meta(stage)
        config = await load_config(self.sheet_id)
        tid = config["ACTIVE_TOURNAMENT_ID"]
        async with _locks[(self.sheet_id, tid)]:
            ...
            round_row.update(
                ...
                notes="Fixed knockout bracket; organizer approval required before publication",
            )
            generated = []
            ...
            rounds = [dict(row) for row in old_rounds] + [round_row]
            matches = [dict(row) for row in old_matches] + generated
            await self.repository.persist_state(rounds, matches, previous_rounds=old_rounds, previous_matches=old_matches)
            await self._audit(tid, actor_id, "knockout_preview_generated", {"stage": stage}, now)
    ...
    async def _audit(self, tid, actor, event, details, now):
        try:
            await self.registration_repository.append_audit(
                dict(event_id=str(uuid4()), tournament_id=tid, event_type=event,
                     actor_discord_user_id=str(actor), target_discord_user_id="",
                     details=json.dumps(details, sort_keys=True, separators=(",", ":")), created_at_utc=now)
            )
        except Exception:
...
def _competitive_ties(top8, standings):
    affected = []
    for index, entry in enumerate(top8):
        if entry.tied:
            affected.append(entry)
        if index == 7 and len(standings) > 8 and standings[8].rank == entry.rank:
            affected.extend([entry, standings[8]])
    unique = {}
...
def _other_player(row, winner_id):
    ...
    return b if winner_id == a else a
...
def _validate_knockout_shape(stage, matches):
    ...
    if len(matches) != expected:
        raise RegistrationError(f"{_meta(stage)['name']} must contain exactly {expected} matchup(s)")
    seen = set()
    for row in matches:
        a = _text(row.get("player_a_discord_user_id"))
        b = _text(row.get("player_b_discord_user_id"))
        if not a or not b or a == b or a in seen or b in seen:
            raise RegistrationError("Knockout preview contains an invalid or duplicate player placement")
        seen.update((a, b))
