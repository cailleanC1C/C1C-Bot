...
def _is_in_target(message: Any, target: Any) -> bool:
    message_channel = getattr(message, "channel", None)
    return getattr(message_channel, "id", None) is not None and getattr(
        message_channel, "id", None
    ) == getattr(target, "id", None)


def _has_plausible_server_rules_embeds(message: Any) -> bool:
    embeds = getattr(message, "embeds", None) or []
    if not embeds:
        return False
    if _validate_embed_payload(list(embeds)):
        return False
    return any(
        (getattr(embed, "title", None) or getattr(embed, "description", None))
        for embed in embeds
    )


def _legacy_managed_message_matches(
    message: Any, bot_id: int | None, target: Any, keys: set[str] | None = None
) -> bool:
    return (
        _is_bot_authored(message, bot_id)
        and _is_in_target(message, target)
        and is_feature_message(message, keys)
    )


def _stored_managed_message_matches(
    message: Any, bot_id: int | None, target: Any, keys: set[str] | None = None
) -> bool:
...
async def _iter_feature_messages(target: Any, bot_id: int | None) -> list[Any]:
    ...
    async for message in history(limit=500):
        if _legacy_managed_message_matches(message, bot_id, target):
...
async def _delete_old_stored_messages(
    ...
    for row in rows:
        ...
        if state is FetchState.UNKNOWN:
            ...
            continue
        if msg is None or not _stored_managed_message_matches(
            msg, bot_id, target, _cleanup_keys_for_row(row)
        ):
            continue
        seen.add(old_id)
        try:
    ...
    for group in groups:
        ...
        try:
            ...
            if (
                ...
                and msg is not None
                and _stored_managed_message_matches(
                    msg,
        ...
        try:
            ...
            if not row.message_id:
                ...
                try:
                    ...
                    summary.skipped += 1
            elif msg is not None and _stored_managed_message_matches(
                msg, getattr(getattr(bot, "user", None), "id", None), target, {row.key}
            ):
                try:
                    await msg.delete()
                    await _write_message_id(tab, header_map, row, "")
