...
- **Behavior.** On every run the watcher reads rows from the configured cleanup tab, validates each row, resolves target metadata, and applies the selected cleanup mode to the configured target's own message history for supported thread and channel rows. Channel rows do not discover or traverse child threads automatically. Pinned messages remain untouched. Webhook-authored messages are only eligible for deletion in the explicit `bot_and_webhook_messages_only` and `bot_webhook_messages_and_commands` modes.
- **Logging.** Each run emits a single summary line: `🧹 cleanup run complete: trigger=<scheduled_or_manual|startup_validation> checked_rows=<N> dry_run=<bool> writeback=<bool> deleted=<M> candidates=<C> skipped=<S> errors=<E>`. Rows that read history but find zero candidates also emit one compact aggregate diagnostic with author/source, webhook, content, prefix, role, and bounded top-author counters. WARN lines accompany missing/invalid sheet configuration, fetch, permission, or delete issues without logging every message.
