...
### Thread keepalive
- **Sheet Config.** Thread keepalive is sheet-driven only. The required Config
  keys are `HOUSEKEEPING_KEEPALIVE_ENABLED`, `HOUSEKEEPING_KEEPALIVE_TAB`,
  `HOUSEKEEPING_KEEPALIVE_DEFAULT_MESSAGE`,
  `HOUSEKEEPING_KEEPALIVE_STALE_AFTER_HOURS`, and
  `HOUSEKEEPING_KEEPALIVE_RUN_EVERY_HOURS`. If any required key is missing or
  invalid, keepalive is not scheduled and no legacy ENV fallback is used.
