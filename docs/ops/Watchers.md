...
| **Cleanup watcher** | `modules.housekeeping.cleanup` | Every `HOUSEKEEPING_CLEANUP_RUN_EVERY_HOURS` hours | Reads Mirralith cleanup targets from the configured cleanup tab and applies the selected cleanup mode. | Summary `🧹 cleanup run complete: trigger=<scheduled_or_manual|startup_validation> checked_rows=<N> dry_run=<bool> writeback=<bool> deleted=<M> candidates=<C> skipped=<S> errors=<E>` posted to the ops log channel. | Feature Toggle `HOUSEKEEPING_CLEANUP_ENABLED`; Config `HOUSEKEEPING_CLEANUP_TAB`, `HOUSEKEEPING_CLEANUP_RUN_EVERY_HOURS`, `HOUSEKEEPING_CLEANUP_DRY_RUN`. |
...
- **Behavior.** On every run the watcher reads rows from the configured cleanup tab, validates each row, resolves target metadata, and applies the selected cleanup mode to the configured target's own message history for supported thread and channel rows. Channel rows do not discover or traverse child threads automatically. Pinned messages remain untouched. Webhook-authored messages are only eligible for deletion in the explicit `bot_and_webhook_messages_only`, `bot_webhook_messages_and_commands`, and `automod_system_and_webhook_messages_only` modes. AutoMod/system moderation alerts are only eligible in the explicit `automod_system_messages_only` and `automod_system_and_webhook_messages_only` modes.
- **Logging.** Each run emits a single summary line: `🧹 cleanup run complete: trigger=<scheduled_or_manual|startup_validation> checked_rows=<N> dry_run=<bool> writeback=<bool> deleted=<M> candidates=<C> skipped=<S> errors=<E>`. Rows that read history but find zero candidates also emit one compact aggregate diagnostic with author/source, webhook, system, AutoMod, message-type, content, prefix, role, and bounded top-author counters. WARN lines accompany missing/invalid sheet configuration, fetch, permission, or delete issues without logging every message.
...
The Wednesday approval and manual `!leagues post` routes share one pipeline. After sheet/config validation, an append-only cluster-history capture runs before the first image export/send. Approval retries reuse the durable approval season/week; manual runs use the current ISO year/week. Capture failures (including alias collisions, negative cumulative deltas, and append-only conflicts) post a failure status and suppress every league board and announcement. A posting failure after capture leaves history intact so a retry can deduplicate it.
